...
with st.sidebar:
    ...
    customer_id = st.text_input("Vectara Customer ID", value=os.getenv("VECTARA_CUSTOMER_ID", ""))
    api_key = st.text_input("Vectara API Key", value=os.getenv("VECTARA_API_KEY", ""))
    corpus_id = st.text_input("Vectara Corpus ID", value=str(os.getenv("VECTARA_CORPUS_ID", "")))
    openai_api_key = st.text_input("OpenAI API Key", value=os.getenv("OPENAI_API_KEY", ""))
    submit_button = st.button("Submit")

# Constants
CUSTOMER_ID = customer_id if customer_id else os.getenv("VECTARA_CUSTOMER_ID")
API_KEY = api_key if api_key else os.getenv("VECTARA_API_KEY")
# Assuming CORPUS_ID should be an integer
CORPUS_ID = int(corpus_id) if corpus_id else int(os.getenv("VECTARA_CORPUS_ID", 0))  
...
prompt = PromptTemplate.from_template(
    """You are a professional and friendly Legal Consultant and you are helping a client with a legal issue. 
    The client is asking you for advice on a legal issue. Just explain him in detail the answer and nothing else. 
    This is the issue: {issue} 
    To assist him with his issue, you need to know the following information: {knowledge} 
    """
)
runnable = prompt | ChatOpenAI(streaming=True, callbacks=[StreamingStdOutCallbackHandler()], 
